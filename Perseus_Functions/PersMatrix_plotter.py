...
if __name__ == '__main__':
    ...
    df_new[x_input] = Tf_float(x_input)
    df_new[c_input] = Tf_float(c_input)
    df_new[s_input] = M_size * Tf_float(s_input)
    df_new.dropna(inplace=True) # Drop the rows with (NaN)
    ##df_new.to_csv('df_GOMF_dropped.txt', index=False, sep='\t') ### Create a file to check
